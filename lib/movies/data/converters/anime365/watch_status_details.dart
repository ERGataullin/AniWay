...
abstract class WatchStatusDetailsConverterAnime365 {
  static WatchStatusDetails fromHtml(Element element) {
    ...
    final String? scoreValue = element
        .querySelector('select#UsersRates_score > option[selected="selected"]')
        ?.attributes['value'];
    final int? score = scoreValue == null ? null : .parse(scoreValue);

    final String? episodesCountValue = element
        .querySelector('input#UsersRates_episodes')
        ?.attributes['value'];
    final int episodesCount = episodesCountValue == null
        ? 0
        : .parse(episodesCountValue);

    final String? comment = element
        .querySelector('textarea#UsersRates_comment')
        ?.innerHtml;
    ...
    return WatchStatusDetails(
      switch (status) {
        ...
        'Брошено' => .dropped,
        null => null,
        final Object? value => throw UnsupportedError(
          'Unsupported $WatchStatus: $value',
        ),
      },
      score: score,
